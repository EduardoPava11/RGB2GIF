...
public actor OctreeColorQuantizer {
    ...
    /// Quantize with attention weights from dual KataGo Q-K-V mechanism
    ///
    /// The attention weights (729 values) determine which macro-cells are most
    /// important for color accuracy. Colors from high-weight cells are prioritized
    /// in the palette, while low-weight cells share colors.
    ///
    /// - Parameters:
    ///   - pixels: 729 RGB centroids from TensorCube729
    ///   - weights: 729 attention weights from DualPlayerAttention
    ///   - maxColors: Maximum palette size (default 256)
    /// - Returns: Attention-weighted palette
    public func quantizeWithAttention(
        _ pixels: [(r: UInt8, g: UInt8, b: UInt8)],
        weights: [Float],
        maxColors: Int = 256
    ) async -> [UInt32] {
        guard pixels.count == weights.count else {
            quantizerLogger.warning("Attention weights count (\(weights.count)) doesn't match pixels (\(pixels.count)), using uniform weights")
            return await quantizeFromPixels(pixels, maxColors: maxColors)
        }

        quantizerLogger.info("Quantizing \(pixels.count) pixels with attention weights...")

        reset()

        // Build octree with attention-weighted colors
        // Higher attention weight = add color multiple times for more influence
        let maxRepetitions = 10  // Maximum times a color can be added

        for (index, pixel) in pixels.enumerated() {
            let weight = weights[index]

            // Convert weight to repetition count (1 to maxRepetitions)
            // Weight range is typically 0 to ~0.05 for normalized weights
            let normalizedWeight = min(1.0, weight * 50)  // Scale up small weights
            let repetitions = max(1, Int(normalizedWeight * Float(maxRepetitions)))
        ...
        quantizerLogger.info("Built attention-weighted octree with \(self.leafNodes.count) unique colors")
